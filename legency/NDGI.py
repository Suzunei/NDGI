...
==========================================
Paper-aligned demo implementing the core NDGI pipeline from reference.md:

  1. Hybrid feature maps (tri-plane + 3D voxel)        — Section 3.3
  2. BC simulation for F_uv_2d & F_uvt_3d               — Section 4.2
  3. 8-bit quantization noise for F_ut_2d & F_vt_2d     — Section 4.1
  4. Gamma correction + per-channel mean normalization  — Section 5
  5. Two-phase training (main + MLP fine-tune)          — Section 5
  6. Lightweight MLP decoder (2×hidden, GELU)            — Section 3.3

      (u,v,t)  ──→  [F_uv_2d, F_uvt_3d, F_ut_2d, F_vt_2d, γ(t)]  ──→  MLP  ──→  RGB
"""
...
def _gaussian_kernel(size: int, sigma: float, device: torch.device) -> torch.Tensor:
    """Create a 2D circular-symmetric Gaussian kernel (standard SSIM uses σ=1.5, 11×11)."""
    coords = torch.arange(size, device=device, dtype=torch.float32) - (size - 1) / 2.0
    g = torch.exp(-(coords ** 2) / (2 * sigma ** 2))
    g = g.unsqueeze(1) * g.unsqueeze(0)          # (size, size)
    g = g / g.sum()
...
def compute_ssim(img1: torch.Tensor, img2: torch.Tensor,
    ...
    Args:
        window_size: should be ~min(H,W)//8 rounded up to odd (default 11 for ≥88px)
        sigma: Gaussian σ (standard = 1.5)
    """
    K1, K2 = 0.01, 0.03
    C1 = (K1 * data_range) ** 2
    C2 = (K2 * data_range) ** 2
...
def generate_synthetic_lightmaps(num_frames: int = 4, h: int = 16, w: int = 16,
    ...
    """
    Generate temporally-varying lightmaps: moving Gaussian spot + checkerboard background.

    Preprocessing (paper Section 5):
      (a) gamma correction:   L' = L_raw ^ (1/gamma)
      (b) per-channel mean normalization per time step:  L'' = L' / (2 * mean[L'])

    Returns
    -------
    lightmaps_raw  : (T, H, W, 3)   — original  (for visualization)
    lightmaps_norm : (T, H, W, 3)   — training target
    norm_stats     : (T, 3)         — per-channel mean  (for denormalization at inference)
    """
    frames = []
    ...
    # ---- (a) Gamma correction (enhances dark details) ----
...
# ═══════════════════════════════════════════════════════════════
#  2. Positional Encoding  (paper Section 3.3 — Step 2)
# ═══════════════════════════════════════════════════════════════

def positional_encoding(x: torch.Tensor, num_freqs: int) -> torch.Tensor:
    """
    Generic Fourier feature encoding for any scalar input ∈ [0, 1].

    γ(x) = [sin(2⁰πx), cos(2⁰πx), sin(2¹πx), cos(2¹πx), …, sin(2^{F-1}πx), cos(2^{F-1}πx)]
...
class NDGI(nn.Module):
    """
    Hybrid feature map decomposition + BC simulation + lightweight MLP.

    Feature maps (paper Table in Section 3.3, scaled for demo):
      F_uv_2d   :  (ch_uv,  H,     W)       — spatial static details
      F_uvt_3d  :  (ch_uvt, T,     H//2, W//2) — spatiotemporal joint variation
      F_ut_2d   :  (ch_ut,  H,     T)       — u-t plane
      F_vt_2d   :  (ch_vt,  W,     T)       — v-t plane

    Decoder:  G_Φ : R^(ch_uv+ch_uvt+ch_ut+ch_vt+2·F_t+4·F_uv) → R³
    """
    ...
    def __init__(self, H: int = 16, W: int = 16, T: int = 4,
                 ...
                 hidden_size: int = 32,
                 num_time_freqs: int = 3, num_uv_freqs: int = 4,
                 block_size: int = 4, uvt_scale: int = 2):
        """
        uvt_scale: spatial downscale factor for F_uvt_3d (2 → H//2 × W//2, 4 → H//4 × W//4)
        """
        super().__init__()
        ...
        self.quant_alpha = 1.0 / 256.0                         # paper Eq. 6

        # ---------- MLP decoder ----------
        # PE: 2*F_t for time + 2*F_uv for u + 2*F_uv for v
        pe_dim = 2 * num_time_freqs + 4 * num_uv_freqs
        ...
        self.num_uv_freqs = num_uv_freqs

    # ------------------------------------------------------------------
    #  BC reconstruction  (paper Eq. 7–8)
    # ------------------------------------------------------------------
    @staticmethod
    def _bc_reconstruct(H: int, W: int, bc_e: torch.Tensor,
                        bc_w: torch.Tensor, blk: int) -> torch.Tensor:
        """
        f_p = (1 − w_p)·e₁ + w_p·e₂    for p = 1…16
        ...
        texels = texels.permute(4, 0, 2, 1, 3).contiguous()      # (C, bh, blk, bw, blk)
        return texels.reshape(C, H, W)

    # ------------------------------------------------------------------
    #  Bilinear / trilinear sampling helpers
    # ------------------------------------------------------------------
    @staticmethod
    def _sample_2d(feat: torch.Tensor, u: torch.Tensor,
                   v: torch.Tensor) -> torch.Tensor:
        """feat: (C, H, W) or (B, C, H, W)  →  (batch, C)"""
        B = u.shape[0]
        if feat.dim() == 3:
    ...
    def _sample_3d(feat: torch.Tensor, u: torch.Tensor,
        ...
        """feat: (C, D, H, W) or (B, C, D, H, W)  →  (batch, C)"""
        B = u.shape[0]
        if feat.dim() == 4:
    ...
    def forward(self, u: torch.Tensor, v: torch.Tensor, t: torch.Tensor,
        ...
        # --- ③  F_ut_2d  (8-bit quantization noise) ---
        V_ut = self._sample_2d(self.F_ut_2d, t, u)              # map: x↔T, y↔H
        if add_quant_noise:
            V_ut = V_ut + (torch.rand_like(V_ut) - 0.5) * self.quant_alpha

        # --- ④  F_vt_2d  (8-bit quantization noise) ---
        V_vt = self._sample_2d(self.F_vt_2d, t, v)              # map: x↔T, y↔W
        if add_quant_noise:
            V_vt = V_vt + (torch.rand_like(V_vt) - 0.5) * self.quant_alpha

        # --- ⑤  γ(t), γ(u), γ(v)  Fourier features ---
        gamma_t = positional_encoding(t, self.num_time_freqs)     # (B, 2·F_t)
        gamma_u = positional_encoding(u, self.num_uv_freqs)       # (B, 2·F_uv)
        gamma_v = positional_encoding(v, self.num_uv_freqs)       # (B, 2·F_uv)

        # --- ⑥  Concat → MLP → RGB ---
        x = torch.cat([V_uv, V_uvt, V_ut, V_vt, gamma_t, gamma_u, gamma_v], dim=-1)
...
# ═══════════════════════════════════════════════════════════════
#  4. Training  (paper Section 5)
# ═══════════════════════════════════════════════════════════════

def train(model: NDGI,
    ...
    Phase 2 — freeze feature maps, fine-tune decoder only  (lower lr).
    """
    ...
    # Pre-build lightmap grid for bilinear GT sampling: (T, 3, H, W) for grid_sample
    ...
    for epoch in range(epochs_phase1 + epochs_phase2):
        ...
        # Bilinear GT sampling (time): interpolate between adjacent frames
        t_scaled = t * (T - 1)                          # (batch,)  in [0, T-1]
        t0 = t_scaled.floor().long().clamp(0, T - 1)
        t1 = (t0 + 1).clamp(0, T - 1)
        wt = (t_scaled - t0.float()).view(-1, 1, 1, 1)  # (batch, 1, 1, 1)

        lm0 = lightmaps_4d[t0]                           # (batch, 3, H, W)
        lm1 = lightmaps_4d[t1]                           # (batch, 3, H, W)

        # Bilinear GT sampling (spatial): use grid_sample
        grid_uv = torch.stack([u * 2 - 1, v * 2 - 1], dim=-1).view(batch_size, 1, 1, 2)
        gt0 = F.grid_sample(lm0, grid_uv, mode='bilinear', align_corners=False)  # (batch, 3, 1, 1)
        gt1 = F.grid_sample(lm1, grid_uv, mode='bilinear', align_corners=False)
        gt = (gt0 * (1 - wt) + gt1 * wt).reshape(batch_size, 3)  # trilinear interp
...
def visualize(lightmaps_raw: torch.Tensor, pred_raw: List[torch.Tensor],
    ...
    ax.axvline(800, color='red', ls='--', alpha=0.5, label='Phase 2')
    ...
    fig.tight_layout()
    fig.savefig('NDGI_results.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("\nVisualization saved → NDGI_results.png")
...
def main():
    ...
    cfg = {
        ...
        'data': {
            ...
            'H': 128,                # lightmap 高度
            'W': 128,                # lightmap 宽度
            'gamma': 2.2,           # gamma correction 系数
        },
        # ── 模型：特征图 ──
        'feature': {
            'ch_uv':  2,            # F_uv_2d  空间静态特征通道
            'ch_uvt': 2,            # F_uvt_3d 时空体素特征通道
            'ch_ut':  2,            # F_ut_2d  u-t 平面特征通道
            'ch_vt':  2,            # F_vt_2d  v-t 平面特征通道
            'block_size': 8,        # BC 压缩块大小 (4→8 压缩率↑4×, 质量↓)
            'uvt_scale': 2,         # F_uvt_3d 空间下采样 (2→4 压缩率↑4×, 质量↓)
        },
        # ── 模型：MLP 解码器 ──
        'decoder': {
            'hidden_size':      32,  # MLP 隐藏层宽度
            'num_time_freqs':   3,    # 时间 PE 频率数  → γ(t) 维度 = 2·F_t
            'num_uv_freqs':     4,    # 空间 PE 频率数  → γ(u)+γ(v) 维度 = 4·F_uv
        },
        # ── 训练 ──
        'train': {
            'epochs_phase1': 2000,    # Phase 1: 全参数训练
            'epochs_phase2': 500,     # Phase 2: 冻结特征图, fine-tune MLP
            'batch_size':    2048,
            'lr':            1e-3,
        },
    }
    ...
    print(f"  Decoder        {params_dict['decoder']:5d}")
    print(f"  Total    raw {params_dict['total_raw']:5d}  →  BC {params_dict['total_bc']:5d}   ({params_dict['total_raw']/params_dict['total_bc']:.2f} : 1)\n")

    # ---- Train ----
    print(f"Training (Phase 1: {t['epochs_phase1']} epochs + Phase 2: {t['epochs_phase2']} epochs) …")
    loss_hist = train(model, norm,
                      epochs_phase1=t['epochs_phase1'],
                      epochs_phase2=t['epochs_phase2'],
    ...
    # ---- Visualize ----
    visualize(raw, pred_raw, loss_hist, psnr_list, ssim_list, params_dict)
